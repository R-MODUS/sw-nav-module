...
def _build_ekf(context):
    ...
    if global_params_file and os.path.exists(global_params_file):
        with open(global_params_file, "r", encoding="utf-8") as f:
            root = _deep_merge(root, yaml.safe_load(f) or {})
    ...
    wheel_odom_cfg = params.get("wheel_odom", {"enabled": True, "type": "odom", "topic": "/odom"})
    lidar_odom_cfg = params.get("lidar_odom", {"enabled": False, "type": "odom", "topic": "/odom_lidar"})
    imu_cfg = params.get("imu", {})
    flow_cfg = params.get("flow_sensor", {})

    imu_cfg.setdefault("type", "imu")
    imu_cfg.setdefault("enabled", True)
    flow_cfg.setdefault("type", "twist")
    flow_cfg.setdefault("enabled", True)
    flow_cfg.setdefault("topic", "/visual_flow/data")
