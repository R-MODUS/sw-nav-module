...
def _create_robot_state_publisher(context):
    ...
    base_config_path = _resolve_config_path(
        LaunchConfiguration("base_config_path").perform(context)
    )
    override_config_path = _resolve_config_path(
        LaunchConfiguration("override_config_path").perform(context)
    )
    ...
    chassis_contents = os.path.join(
        get_package_share_directory("rmodus_chassis"),
        "urdf",
        "chassis_contents.urdf.xacro",
    )
